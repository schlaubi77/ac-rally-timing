...
Version 1.3 BETA 3 (29/05/2023)

#####################################################################################################################################

The app has 2 main functions: start position verification and delta timing.

Start position: The app wil help in finding proper starting position, and alert if a lap will be invalidated by AC Server Manager when a speed trap is set on the startline.
...
By default the app will indicate starting position as OK when stopped at < 0.5m, but the detected speed on the line is what counts!
This distance can be adjusted in the client settings.
...
- Delta timing: 
  * Reference file can be chosen (from same car, other cars, or other driver/car combos)
  (((* Works during replays))))
  * Weather conditions, startline speed are registered in the reference files
  * By default all runs are saved. In settings the number of times kept per driver/car combo can be limited (not working yet)
